...
# Usage:
# ./install_eza.sh <dotfiles_base_path> <config_base_path> <operating_system>
#
# Example:
# ./install_eza.sh ~/dotfiles ~/.config mac
# or
# ./install_eza.sh ~/dotfiles ~/.config arch
...
echo "Setting up eza for OS: $OPERATING_SYS"

# --- Check and install eza ---
if [[ "$OPERATING_SYS" == "mac" ]]; then
  if ! command -v eza >/dev/null 2>&1; then
    echo "eza not found. Installing with Homebrew..."
    if ! brew install eza; then
      echo "Error: Failed to install eza with Homebrew" >&2
      exit 1
    fi
  else
  ...
  if ! check_sudo_available; then
    echo "Error: sudo is required for installing eza" >&2
    exit 1
  fi
  
  if ! pacman -Q eza >/dev/null 2>&1; then
    echo "eza not found. Installing with pacman..."
    if ! sudo pacman -S --needed --noconfirm eza; then
      echo "Error: Failed to install eza with pacman" >&2
      exit 1
    fi
  else
    echo "eza already installed."
