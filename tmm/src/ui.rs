...
pub fn render(app: &mut App, frame: &mut Frame) {
    ...
    // Compute the strings that will be displayed (one per row) for other sessions
    ...
    // Calculate heights for the current session box and sessions list
    let current_session_height: u16 = if app.current_session.is_some() { 3 } else { 0 };
    let sessions_list_height: u16 = if app.sessions.is_empty() { 0 } else { 4 + app.sessions.len() as u16 };
    ...
    let chunks = Layout::default()
        ...
        .constraints([
            Constraint::Fill(1),
            Constraint::Length(current_session_height),
            Constraint::Max(sessions_list_height),
            Constraint::Length(1),
        ])
        .split(frame.size());
    ...
    if let Some((name, desc)) = &app.current_session {
        let current_text = format!("★ {:>2$} {}", name, desc, width);
        let current_session_widget = Paragraph::new(current_text)
            .style(Style::default().fg(Color::Yellow))
            .block(
                Block::bordered()
                    .border_style(Style::default().fg(Color::Yellow))
            );
        frame.render_widget(current_session_widget, chunks[1]);
    }
    ...
    match app.state {
        AppState::Deleting => {
            ...
            if let Some((name, _)) = app.sessions.get(app.selected_session) {
                ...
                display_popup_centered(frame, &chunks[2], "Confirm Delete",
        ...
        AppState::WarnNested => {
            display_popup_centered(frame, &chunks[2], "Error",
        ...
        AppState::Renaming => {
            ...
            if let Some(textarea) = &app.rename_session_ta {
                let title = if app.renaming_current {
                    "Rename Current Session"
                } else {
                    "New Session Name"
                };
                display_prompt_centered(frame, &chunks[2], textarea, title)
            }
        }
        AppState::NewSession => {
            // Render text input dialog to get the desired new name
            if let Some(textarea) = &app.new_session_ta {
                display_prompt_centered(frame, &chunks[2], textarea, "New Session Name")
            }
        }
        AppState::SessionsSearch => {
            // Render text input dialog to get the desired new name
            if let Some(textarea) = &app.search_session_ta {
                ...
                // Compute the rect
                let Rect{x, y, width, height} = chunks[2];
    ...
    // render it
    frame.render_widget(Line::from(hotkey_spans), chunks[3]);
}
